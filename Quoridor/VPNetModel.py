...
class QuoridorPolicyValueNet(PolicyValueNet):
    def __init__(self, board_width, model_file=None):
        ...
        Y = tf.layers.batch_normalization(Y, axis=3)
        Y = tf.nn.relu(Y)
        self.action_conv_flat = tf.reshape(Y, [-1, 4, board_width * board_width], name="action_conv_flat")  # ?,4,49
        self.action_fc = tf.layers.dense(inputs=self.action_conv_flat, units=board_width * board_width,
                                         activation=tf.nn.log_softmax, name="action_fc")  # ?,4,49
        # 4. v值输出
        Y = tf.layers.conv2d(inputs=X, filters=1, kernel_size=[1, 1], padding="same", data_format="channels_last")
        Y = tf.layers.batch_normalization(Y, axis=3)
        Y = tf.nn.relu(Y)  # ?,7,7,1
        self.evaluation_conv_flat = tf.reshape(Y, [-1, 1 * board_width * board_width], name="evaluation_conv_flat")  # ?,4,49
        self.evaluation_fc1 = tf.layers.dense(inputs=self.evaluation_conv_flat, units=64, activation=tf.nn.relu, name="evaluation_fc1")  # ?,4,49
        self.evaluation_fc2 = tf.layers.dense(inputs=self.evaluation_fc1, units=1, activation=tf.nn.tanh, name="evaluation_fc2")  # ?,1
...
class TictactoePolicyValueNet(PolicyValueNet):
    ...
    def policy_value_fn(self, CurrentState
                        , legal_ActionList=[231, 123, 23, 134]):
        legal_action = []
        legal_location = []
        for Action in legal_ActionList:
            legal_action.append((Action // 100) % 10)
